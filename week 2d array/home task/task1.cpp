...
struct Enemy {
    int x, y;
    int type; // 1=normal, 2=fast, 3=tank
    int health;
    int shootCooldown;
    int moveCounter;
    int active; // Added active member
};
...
void initializeLevel() {
    ...
    for(int i = 0; i < numEnemies; i++) {
        ...
        enemies[i].x = 5 + (i * 3);
        enemies[i].y = 3 + (i / 5);
        enemies[i].shootCooldown = rand() % 30;
        enemies[i].moveCounter = 0;
        
        // Set enemy types based on level
        if(currentLevel == 1) {
            enemies[i].type = 1;
            enemies[i].health = 30;
        }
...
void drawHealthAndScore() {
    gotoxy(2, HEIGHT + 2);
    cout << "Health: " << playerHealth << "%  Lives: ";
    for(int i = 0; i < playerLives; i++) cout << char(3) << " ";
    cout << "  Score: " << score << "  Level: " << currentLevel;
...
int main() {
    ...
    if(playerLives > 0 && currentLevel == 3 && enemiesAlive == 0) {
        system("cls");
        gotoxy(12, 10);
        cout << "YOU WIN!";
        gotoxy(13, 11);
